...
Ext.ux.FileTreePanel = Ext.extend(Ext.tree.TreePanel, {
	 ...
	 */
	,url:'os/filemanager/filetree.php'
	// }}}
...
//				,menu:this.getContextMenu()
			...
			if(true === o.success) {
				switch(options.params.cmd) {
					...
					case 'zip':
						this.updateCls(options.node, options.params.oldname);
						if(true !== this.eventsSuspended) {
							this.fireEvent('zip', this, options.node, options.params.oldname);
						}
					break;
					case 'unzip':
						this.updateCls(options.node, options.params.oldname);
	...
	,showContextMenu:function(node) {
		...
		menu.setItemDisabled('zip', this.readOnly || node === this.root || node.disabled);
		menu.setItemDisabled('unzip', this.readOnly || node === this.root || node.disabled);
